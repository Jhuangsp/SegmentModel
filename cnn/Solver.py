...
class Solver(object):
    ...
    def train(self, args, DataLoader, net):
        ...
        with tf.Session(graph=net.train_graph) as sess:
            ...
            for epoch_i in range(1, args.epochs+1):
                ...
                for batch_i, (sources_batch, targets_batch, num_batch) in enumerate(train_batch_generator):
                    ...
                    if batch_i % args.display_step == 0:
                        ...
                        answer_logits = np.zeros((length, args.out_band)) # model result buffer
                        for i in range(length):
                            indata = infer_data[i:i+args.in_frames, :]
                            LR, validation_result= sess.run([optimizer._lr, training_logits], { input_data: np.array([indata])}) # batch size 1
                            # answer_logits[i] = validation_result[0,0]
                            answer_logits[i] = validation_result[0]
                        ...
                        # SSE
                        valuefull = infer_targ.reshape(3,-1)[-1] != 0
                        valueless = infer_targ.reshape(3,-1)[-1] == 0
                        num_valuefull = valuefull.sum()
                        num_valueless = valueless.sum()
                        total = num_valuefull + num_valueless
                        penalty = valuefull * num_valueless/total + valueless * num_valuefull/total
                        Valid_SSE = ((a_logits - infer_targ.reshape(3,-1)[-1])**2*penalty).sum()
                        print(' - Epoch {:>3}/{} | Batch {:>4}/{} | LR: {:>4.3e} | Train Loss: {:>6.3f} | Valid SSE: {:>6.3f}'
                              .format(epoch_i,
                                      args.epochs, 
                                      batch_i, 
                                      num_batch,
            ...
            sum_writer = tf.summary.FileWriter('C:\\Users\\astor\\code\\SegmentModel\\log', graph=net.train_graph)
            sum_writer.close()
        pass

    def test(self, args, DataLoader, demo=False):
        print('==> Start Testing...\n')
        ...
        if not demo:
            ...
            activity = 'DEMO'
            infer_data = DataLoader.source_data['DEMO']
        ...
        with tf.Session(graph=loaded_graph) as sess:
            ...
            for i in range(length):
                indata = infer_data[i:i+args.in_frames, :]
                answer_logits[i] = sess.run(logits, { input_data: np.array([indata])})[0] # first output, first batch
        ...
        if not demo:
            ...
            draw(args=args, result=a_logits, gt=None)

    def demo(self, args, DataLoader):
        print('==> Start Testing...\n')
        activity = 'DEMO'
        infer_data = DataLoader.source_data['DEMO']
